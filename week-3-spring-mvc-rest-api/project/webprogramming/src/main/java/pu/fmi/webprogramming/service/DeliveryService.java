...
import pu.fmi.webprogramming.contoller.rest.DeliveryApi;
...
public class DeliveryService implements DeliveryServiceInterface {
  ...
  public Delivery assignCourier(Long id, Long courierId) {
    ...
    if (delivery == null) throw new DeliveryCustomException("Delivery not found");

    if (courier == null) throw new DeliveryCustomException("Courier not found");

    if (!courier.isAvailable()) throw new DeliveryCustomException("Courier is not available");
